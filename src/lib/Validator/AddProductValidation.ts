...
export const AddProductValidation = z.object({
  ...
  genre:z.string(),
  product_thumbnail: z.instanceof(File).refine((file) => {
    return file.size <= 10 * 1024 * 1024; // Validate file size is less than 10 MB
  }, {message:'Thumbnail must be less than 10 MB'}),
  product_pics: z
      .array(z.instanceof(File))
      .refine((files) => files.length > 0, {message: 'At least one image is required.'})
      .refine((files) => files.every((file) => file.size <= 10 * 1024 * 1024), {message:'Image file size must be less than 10MB'}),
  product_videos: z
      .array(z.instanceof(File))
      .refine((files) => files.length > 0, {message: 'At least one video is required.'})
      .refine((files) => files.every((file) => file.size <= 10 * 1024 * 1024), {message:'Video file size must be less than 10MB'}),
});
